...
public class CountingRooms {
    ...
    static class Solution {
        ...
        public static void floodflill(int row,int col){
            Stack<Pos> front=new Stack<> ();
            front.push (new Pos (row,col));

            while (!front.isEmpty ()){
                Pos current=front.pop ();
                row=current.row;
                col=current.col;

                if(row<0 || row>=rowNum || col<0 ||col>=colNum || building[row][col]=='#' || visited[row][col])
                    continue;

                visited[row][col]=true;

                for(int i=0;i<4;i++){
                    front.add (new Pos (row+R_change[i],col+C_change[i]));
                }
            }
        }
        static class Pos{
