...
public class ChessboardQueens {
    ...
    static class Solution {
        ...
        static String s[];
        static boolean sum[],difference[],c[];
        static int ans=0;

        public void solve(int testNumber, FastInput in, FastOutput out) {
            s=new String[8];
            sum=new boolean[15];
            difference=new boolean[15];
            c=new boolean[8];
            in.readArray (s);
//            debug.debug ("s",s);
//
        ...
        static void dfs(int place){
            ...
            for (int i = 0; i <8; i++) {
                if(s[place].charAt (i)=='.' && !c[i] && !sum[i+place]&& !difference[i-place+7]){
                    c[i]=sum[i+place]=difference[i-place+7]=true;
                    dfs(place+1);
                    c[i]=sum[i+place]=difference[i-place+7]=false;
                }
            }
        }
