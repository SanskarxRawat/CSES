...
public class ShortestRoutesll {
    ...
    static class Solution {
        ...
        //this question is based on flowed warshall algorithm

        public void solve(int testNumber, FastInput in, FastOutput out) {
            int n=in.ri (),m=in.ri (),q= in.ri ();

            long[][] matrix=new long[n][n];
            long INF=(long)1e18;
            for(long[] row:matrix) Arrays.fill(row,INF);

            for(int i=0;i<m;i++){
                int u=in.ri ()-1,v=in.ri ()-1,w=in.ri ();
                ...
                for (int j = 0; j <n; j++) {
                    for (int k = j+1; k <n; k++) {
                        if(matrix[j][i]+matrix[i][k]<matrix[j][k]){
                            matrix[j][k]=matrix[k][j]=matrix[j][i]+matrix[i][k];
                        }
                    }
            ...
            while(q-->0){
                int u=in.ri ()-1,v=in.ri ()-1;
                if(u==v) matrix[u][v]=0;
                if(matrix[u][v]==INF) matrix[u][v]=-1;
                out.prtl (matrix[u][v]);
            }
        }
