...
public class Apartments {
    ...
    static class Solution {
        ...
        public void solve(int testNumber, FastInput in, FastOutput out) {
            ...
            PriorityQueue<Integer> apart=new PriorityQueue<> (m);
            for (int i = 0; i <m; i++) {
                apart.add (in.ri ());
            }
            int count=0;

           while (!req.isEmpty () && !apart.isEmpty ()){
               if((apart.peek ()>=req.peek ()-k) && (apart.peek ()<=req.peek ()+k)){
                   req.poll ();
                   apart.poll ();
                   count++;
               } else if(req.peek ()+k<apart.peek ()){
                   req.poll ();
               }
               else if(req.peek ()-k>apart.peek ()){
                   apart.poll ();
