...
public class Labyrinth {
    ...
    static class Solution {
        ...
        //this implementation do not find the route of shortest path
        public void solve(int testNumber, FastInput in, FastOutput out) {
            ...
            in.readArray (grid);
            int ans=minDistace (grid);
            if(ans<0) out.prtl ("NO");
            else {
                out.prtl ("YES");
            }
        }
        static class Path{
            ...
            int col;
            int distance;
            public Path(int row,int col,int distance){
                this.row=row;
                this.col=col;
        ...
        public static int minDistace(char[][] grid){
            Path source=new Path (0,0,0);
            ...
            queue.add (new Path (source.row,source.col,0));

            visited[source.row][source.col]=true;
            while (!queue.isEmpty ()){
                Path q=queue.remove ();
                if(grid[q.row][q.col]=='B'){
                    return q.distance;
                }
                //moving up
                if(isValid(q.row-1,q.col,grid,visited)){
                    queue.add (new Path (q.row-1,q.col,q.distance+1));
                    visited[q.row-1][q.col]=true;

                }
                //moving left
                if(isValid (q.row,q.col-1,grid,visited)){
                    queue.add (new Path (q.row,q.col-1,q.distance+1));
